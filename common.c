...
void init_app(int argc, char** argv, int* rank, int* size)
{
  ...
  if (*rank == 0) {
    printf("Acquired MPI threading level: ");
    if (acquired == MPI_THREAD_SINGLE)
      printf("MPI_THREAD_SINGLE\n");
    if (acquired == MPI_THREAD_FUNNELED)
      printf("MPI_THREAD_FUNNELED\n");
    if (acquired == MPI_THREAD_SERIALIZED)
      printf("MPI_THREAD_SERIALIZED\n");
    if (acquired == MPI_THREAD_MULTIPLE)
